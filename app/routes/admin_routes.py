...
def get_credential_manager():
    """获取全局凭证管理器实例"""
    return current_app.config.get('credential_manager')
...
def get_qrcode_api(qr_type):
    ...
    try:
        ...
        # 启动后台线程检查二维码状态
        thread = threading.Thread(target=check_qr_status, args=(session_id, current_app._get_current_object()))
        thread.daemon = True
        thread.start()
...
def check_qr_status(session_id, app):
    """后台线程：检查二维码状态"""
            ...
            while attempts < max_attempts and session_id in qr_sessions:
                try:
                    ...
                    if event == QRCodeLoginEvents.DONE:
                        ...
                        if manager:
                            if manager.save_credential(credential):
                                # 强制重新加载凭证到内存
                                manager.force_reload()
                                logger.info("凭证已保存并加载到内存")
                            else:
                                logger.error("保存凭证失败")
                        else:
                            logger.error("凭证管理器未找到，无法保存凭证")
...
def check_credential_status():
    ...
    try:
        manager = get_credential_manager()
        if not manager:
            return jsonify({"valid": False, "error": "凭证管理器未初始化"}), 500

        # 重新加载凭证到内存
        cred = manager.load_credential()
        if not cred:
            return jsonify({"valid": False, "message": "未找到凭证文件"})
        ...
        return jsonify({
            ...
            "expired": is_expired,
            "credential_loaded": manager.credential is not None
        })
    except Exception as e:
...
def refresh_credential():
    ...
    try:
        manager = get_credential_manager()
        if not manager:
            return jsonify({'error': '凭证管理器未初始化'}), 500
        ...
        # 保存刷新后的凭证并更新内存
        if manager.save_credential(cred):
            # 确保内存中的凭证是最新的
            manager.force_reload()
...
def get_credential_info():
    ...
    try:
        ...
        # 返回凭证的基本信息，隐藏敏感信息
        cred_dict = cred.__dict__
        info = {}
        for key, value in cred_dict.items():
            if key.lower() in ['token', 'refresh_token', 'cookie', 'access_token', 'password']:
                # 敏感信息，只显示部分
        ...
        try:
            is_expired = run_async(check_expired(cred))
            can_refresh = run_async(cred.can_refresh())
            info['expired'] = str(is_expired)
            info['can_refresh'] = str(can_refresh)
        except:
            pass
